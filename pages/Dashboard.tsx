...
import { Power, Flame, Timer, Sparkles, Sliders, Battery, Wifi, Brain, MessageSquare, X } from 'lucide-react';
import { DeviceState, HeatLevel, MassageMode, User } from '../types';
...
const Dashboard: React.FC<DashboardProps> = ({ user }) => {
  ...
  const handleHeatChange = () => {
    const levels = [HeatLevel.OFF, HeatLevel.LOW, HeatLevel.HIGH];
    const currentIndex = levels.indexOf(deviceState.heat);
    const nextIndex = (currentIndex + 1) % levels.length;
    setDeviceState(prev => ({ ...prev, heat: levels[nextIndex] }));
  };
